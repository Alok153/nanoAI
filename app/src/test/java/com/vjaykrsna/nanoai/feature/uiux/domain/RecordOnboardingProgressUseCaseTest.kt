...
class RecordOnboardingProgressUseCaseTest {
    @Test
    fun `records dismissed tips and completion flags`() =
        runTest {
            ...
            val intermediateRecord = spy.lastOnboardingRecord ?: fail("Expected onboarding record for dismissal")
            ...
            val updatedPrefs = spy.preferencesFlow.value ?: fail("Expected preferences emission after dismissal")

            @Suppress("UNCHECKED_CAST")
            val dismissedTips = UiUxDomainReflection.getProperty(updatedPrefs, "dismissedTips") as Map<String, Boolean>
            assertThat(dismissedTips).containsExactlyEntriesIn(mapOf("welcome" to true))
            val onboardingComplete = UiUxDomainReflection.getProperty(updatedPrefs, "onboardingCompleted") as Boolean
            ...
            val finalPrefs = spy.preferencesFlow.value ?: fail("Expected preferences emission after completion")
            val finalCompleted = UiUxDomainReflection.getProperty(finalPrefs, "onboardingCompleted") as Boolean
            assertThat(finalCompleted).isTrue()
            @Suppress("UNCHECKED_CAST")
            val finalTips = UiUxDomainReflection.getProperty(finalPrefs, "dismissedTips") as Map<String, Boolean>
    ...
    private fun invokeOnboarding(
        ...
        val candidate =
            instance.javaClass.methods.firstOrNull { method ->
                ...
                ?: fail("Expected (String?, Boolean, Boolean) onboarding method on ${instance.javaClass.name}")

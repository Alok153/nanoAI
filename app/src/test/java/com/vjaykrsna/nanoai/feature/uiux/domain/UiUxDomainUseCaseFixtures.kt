...
internal object UiUxDomainReflection {
    ...
    fun copyUiPreferences(
        ...
        val resolvedDismissed = dismissedTips ?: (getProperty(baseline, "dismissedTips") as Map<String, Boolean>)
    ...
    fun updateLayoutCompact(
        ...
        @Suppress("UNCHECKED_CAST")
        val pinned = getProperty(layout, "pinnedTools") as List<String>
...
internal class UserProfileRepositorySpy {
    ...
    fun asProxy(): Any {
        val repositoryClass = loadClass("com.vjaykrsna.nanoai.core.data.repository.UserProfileRepository")
        return Proxy.newProxyInstance(
            ...
            when {
                ...
                name.contains("onboarding", ignoreCase = true) -> {
                    invocations += name
                    val dismissed = args?.firstOrNull { it is Map<*, *> } as? Map<String, Boolean> ?: emptyMap()
                ...
                name.contains("compact", ignoreCase = true) -> {
                    ...
                    if (enabled == null) {
                        ...
                        preferencesFlow.value =
                            UiUxDomainReflection.copyUiPreferences(
                                preferencesFlow.value,
                                visualDensity = UiUxDomainReflection.visualDensity(if (enabled) "COMPACT" else "DEFAULT"),
                            )
                        layoutSnapshotsFlow.value = layoutSnapshotsFlow.value.map { UiUxDomainReflection.updateLayoutCompact(it, enabled) }
                        Unit
                    }
                }
                else -> {
    ...
    private fun handleObserve(methodName: String): Any =
        when {
            ...
            methodName.contains("State", ignoreCase = true) || methodName.contains("UiState", ignoreCase = true) -> uiStateFlow
            methodName.contains("Preference", ignoreCase = true) || methodName.contains("Ui", ignoreCase = true) -> preferencesFlow
            methodName.contains("Offline", ignoreCase = true) || methodName.contains("Network", ignoreCase = true) -> offlineStatusFlow
...
internal fun instantiateUiUxUseCase(
    ...
    throw IllegalStateException("Unable to instantiate $className with repository/dispatcher test doubles")
}
...
internal class AnalyticsRecorder {
    ...
    fun asProxy(parameterType: Class<*>): Any {
        if (!parameterType.isInterface) {
            throw IllegalStateException("Analytics dependency ${parameterType.name} must be an interface for testing")
        }

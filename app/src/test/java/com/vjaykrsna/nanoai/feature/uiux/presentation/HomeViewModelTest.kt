...
class HomeViewModelTest {
    @Test
    fun stateReflectsRecentActionsOfflineStatus_andTooltipDismissal() =
        runTest {
            ...
            val uiStateSnapshot = UiUxDomainReflection.newUiStateSnapshot(recentActions = listOf("a", "b", "c"))
            ...
            val onboardingRecord = spy.lastOnboardingRecord ?: fail("Expected onboarding record after dismiss")
            assertThat(onboardingRecord.first.keys).contains("home_tools_tip")
        }

    @Test
    fun toolsToggle_andRecentActionUpdateUiState() =
        runTest {
            val spy = UserProfileRepositorySpy()
            val profile = UiUxDomainReflection.newUserProfile(dismissedTips = emptyMap())
            spy.profileFlow.value = profile
    ...
    private fun resolveUiState(viewModel: Any): StateFlow<HomeUiState> {
        ...
        return method.invoke(viewModel) as StateFlow<HomeUiState>
    }

    private fun invokeToggleTools(viewModel: Any) {
        val method =
            viewModel.javaClass.methods.firstOrNull { it.name.contains("toggle", ignoreCase = true) && it.parameterCount == 0 }
                ?: fail("Expected toggleToolsExpanded method on HomeViewModel")
        method.invoke(viewModel)
    }

    private fun invokeRecentAction(
        viewModel: Any,
        actionId: String,
    ) {
        val method =
    ...
    private fun invokeDismissTooltip(viewModel: Any) {
        val method =
            viewModel.javaClass.methods.firstOrNull { it.name.contains("dismiss", ignoreCase = true) && it.parameterCount == 0 }

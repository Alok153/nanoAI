...
class FirstLaunchDisclaimerViewModelTest {
    ...
    fun setUp() {
        ...
        store =
            mockk(relaxed = true) {
                every { privacyPreference } returns preferenceFlow
            }
    }

    @Test
    fun `uiState shows dialog when consent not acknowledged`() =
        runTest {
    ...
    @Test
    fun `onDismiss increments shown count but keeps dialog visible`() =
        runTest {
            val viewModel = FirstLaunchDisclaimerViewModel(store)
    ...
    @Test
    fun `onAcknowledge records consent and hides dialog`() =
        runTest {
            val viewModel = FirstLaunchDisclaimerViewModel(store)

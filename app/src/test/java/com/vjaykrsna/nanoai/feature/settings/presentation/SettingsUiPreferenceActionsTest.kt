...
class SettingsUiPreferenceActionsTest {
  ...
  fun setUp() {
    ...
    actions = SettingsUiPreferenceActions(
      scope = testScope,
      state = state,
  ...
  @Test
  fun `setThemePreference updates state and calls use case`() = testScope.runTest {
  ...
  @Test
  fun `setThemePreference does nothing when same theme`() = testScope.runTest {
  ...
  @Test
  fun `setThemePreference emits error on failure`() = testScope.runTest {
  ...
  @Test
  fun `setCompactMode updates state and calls use case`() = testScope.runTest {
  ...
  @Test
  fun `setCompactMode does nothing when same value`() = testScope.runTest {
  ...
  @Test
  fun `setHighContrastEnabled updates state and calls use case`() = testScope.runTest {
  ...
  @Test
  fun `setHighContrastEnabled does nothing when same value`() = testScope.runTest {
  ...
  @Test
  fun `undoUiPreferenceChange restores previous state`() = testScope.runTest {
  ...
  @Test
  fun `undoUiPreferenceChange does nothing when no snapshot`() = testScope.runTest {
  ...
  @Test
  fun `applyDensityPreference calls setCompactMode`() = testScope.runTest {
    coEvery { toggleCompactModeUseCase.setCompactMode(true) } returns NanoAIResult.success(Unit)

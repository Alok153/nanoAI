...
class ModelDownloadsAndExportUseCaseTest {
    ...
    fun setup() {
        ...
        useCase = ModelDownloadsAndExportUseCase(
    ...
    @Test
    fun `downloadModel should enforce concurrent download limit`() = runTest {
    ...
    @Test
    fun `downloadModel should start immediately when under concurrent limit`() = runTest {
    ...
    @Test
    fun `downloadModel should validate checksum after download completes`() = runTest {
        // Arrange
    ...
    @Test
    fun `downloadModel should fail when checksum mismatch detected`() = runTest {
        // Arrange
        val modelId = "gemini-2.0-flash-lite"
        val expectedChecksum = "abc123def456"
    ...
    @Test
    fun `resumeDownload should restart paused download`() = runTest {
        // Arrange
        val taskId = UUID.randomUUID()
        val pausedTask = createDownloadTask(
            taskId = taskId,
    ...
    @Test
    fun `cancelDownload should clean up partial files and update state`() = runTest {
        // Arrange
        val taskId = UUID.randomUUID()
    ...
    @Test
    fun `deleteModel should fail if model is currently active in session`() = runTest {
        // Arrange
    ...
    @Test
    fun `deleteModel should remove files and update catalog when model inactive`() = runTest {
        // Arrange
    ...
    @Test
    fun `exportBackup should create bundle with personas, credentials, and config`() = runTest {
        // Arrange
    ...
    @Test
    fun `exportBackup should exclude chat history by default`() = runTest {
        // Arrange
    ...
    @Test
    fun `exportBackup should include chat history when requested`() = runTest {
        // Arrange
    ...
    @Test
    fun `exportBackup should warn user about unencrypted export`() = runTest {
        // Arrange
        val exportPath = "/storage/emulated/0/Download/backup.zip"
    ...
    @Test
    fun `getDownloadProgress should emit progress updates`() = runTest {
        // Arrange
        val taskId = UUID.randomUUID()
    ...
    @Test
    fun `getQueuedDownloads should return downloads in QUEUED state`() = runTest {
        // Arrange
    ...
    @Test
    fun `retryFailedDownload should reset error state and restart`() = runTest {
        // Arrange
        val taskId = UUID.randomUUID()
        val modelId = "failed-model"
        val failedTask = createDownloadTask(
            taskId = taskId,
            modelId = modelId,

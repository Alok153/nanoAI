...
class WelcomeViewModelTest {
    @Test
    fun `uiState branches based on onboarding completion`() =
        runTest {
            val repository =
            ...
            val onboardingVisible =
                resolveBoolean(initialState, "showOnboarding", "isOnboarding", "onboardingVisible", "shouldShowOnboarding")
            ...
            repository.preferencesFlow.value = UiUxDomainReflection.newUiPreferences(onboardingCompleted = true)

            advanceUntilIdle()

            val updatedState = stateFlow.value
            val onboardingAfter =
                resolveBoolean(updatedState, "showOnboarding", "isOnboarding", "onboardingVisible", "shouldShowOnboarding")
            assertThat(onboardingAfter).isFalse()
        }

    @Test
    fun `cta interactions emit analytics events`() =
        runTest {
            val repository =
                UserProfileRepositorySpy().apply {
                    val profile = UiUxDomainReflection.newUserProfile(onboardingCompleted = false)
            ...
            assertThat(analytics.events.any { it.contains("get", ignoreCase = true) && it.contains("start", ignoreCase = true) }).isTrue()
            assertThat(analytics.events.any { it.contains("explore", ignoreCase = true) }).isTrue()
        }

    @Test
    fun `skip action becomes gated after invocation`() =
        runTest {
            val repository =
                UserProfileRepositorySpy().apply {
                    val profile = UiUxDomainReflection.newUserProfile(onboardingCompleted = false)
    ...
    private fun instantiateViewModel(
        ...
        constructors.forEach { constructor ->
            ...
            constructor.parameterTypes.forEach { parameter ->
                ...
                when {
                    parameter.isAssignableFrom(observeUseCase.javaClass) || typeName.contains("ObserveUserProfileUseCase") ->
                        args +=
                            observeUseCase
                    parameter.isAssignableFrom(recordUseCase.javaClass) || typeName.contains("RecordOnboardingProgressUseCase") ->
                        args +=
                            recordUseCase
                    parameter.isAssignableFrom(updateThemeUseCase.javaClass) || typeName.contains("UpdateThemePreferenceUseCase") ->
                        args +=
                            updateThemeUseCase
                    parameter.isAssignableFrom(toggleCompactUseCase.javaClass) || typeName.contains("ToggleCompactModeUseCase") ->
                        args +=
                            toggleCompactUseCase
                    typeName.contains("SavedStateHandle") -> args += SavedStateHandle()
                    typeName == CoroutineDispatcher::class.java.name || typeName == "kotlinx.coroutines.CoroutineDispatcher" ->
                        args +=
                            dispatcher
                    ...
                    typeName.contains("Analytics", ignoreCase = true) -> args += analyticsProxy(parameter, analytics)
                    typeName.contains("Navigator", ignoreCase = true) && parameter.isInterface -> args += createInterfaceProxy(parameter)
    ...
    private fun resolveStateFlow(viewModel: Any): StateFlow<Any> {
        ...
        return method.invoke(viewModel) as StateFlow<Any>
    }
    ...
    private fun resolveBoolean(
        ...
        fail("Boolean property ${propertyCandidates.joinToString()} not found on ${instance.javaClass.name}")
    }
    ...
    private fun invokeMethod(
        ...
        val method =
            ...
            } ?: fail("None of the method candidates ${candidates.joinToString()} exist on ${instance.javaClass.name}")
    ...
    private fun analyticsProxy(
        ...
        if (parameter.isInterface) {
            ...
            instantiateDefault(parameter) ?: fail("Analytics dependency ${parameter.name} must have default constructor or be an interface")
        }

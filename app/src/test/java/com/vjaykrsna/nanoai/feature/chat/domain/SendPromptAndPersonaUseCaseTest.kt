...
class SendPromptAndPersonaUseCaseTest {
    ...
    fun setup() {
        ...
        useCase = SendPromptAndPersonaUseCase(
    ...
    @Test
    fun `sendPrompt should use local model when offline and model available`() = runTest {
        // Arrange
        ...
        } returns InferenceResult.Success(
    ...
    @Test
    fun `sendPrompt should fail gracefully when offline and no local model`() = runTest {
        // Arrange
        val threadId = UUID.randomUUID()
    ...
    @Test
    fun `sendPrompt should use cloud API when online and local model unavailable`() = runTest {
        // Arrange
        val threadId = UUID.randomUUID()
    ...
    @Test
    fun `sendPrompt should prefer local model when both local and cloud available`() = runTest {
        // Arrange
        val threadId = UUID.randomUUID()
    ...
    @Test
    fun `sendPrompt should save message to repository with latency metrics`() = runTest {
        // Arrange
        val threadId = UUID.randomUUID()
        val prompt = "Hello AI"
        val personaId = UUID.randomUUID()
    ...
    @Test
    fun `switchPersona should log persona change when continuing thread`() = runTest {
        // Arrange
        val threadId = UUID.randomUUID()
    ...
    @Test
    fun `switchPersona should create new thread when action is START_NEW_THREAD`() = runTest {
        // Arrange
        ...
        // Act
        val result = useCase.switchPersona(
    ...
    @Test
    fun `switchPersona should handle first persona assignment with null previous`() = runTest {
        // Arrange
        val threadId = UUID.randomUUID()
        val newPersonaId = UUID.randomUUID()
    ...
    @Test
    fun `sendPrompt should apply persona temperature settings`() = runTest {
        // Arrange
        val threadId = UUID.randomUUID()
    ...
    @Test
    fun `sendPrompt should store error code when inference fails`() = runTest {
        // Arrange
        val threadId = UUID.randomUUID()
        val prompt = "Test prompt"
        val personaId = UUID.randomUUID()
    ...
    @Test
    fun `getPersonaSwitchHistory should return logs for thread`() = runTest {
        // Arrange
        val threadId = UUID.randomUUID()
        val logs = listOf(
            PersonaSwitchLog(

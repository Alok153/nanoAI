...
class SendPromptAndPersonaUseCaseTest {
  ...
  @Test
  fun sendPrompt_returnsFailureWhenOfflineAndNoLocalModel() = runTest {
  ...
  @Test
  fun sendPrompt_prefersLocalModelWhenAvailable() = runTest {
    val threadId = UUID.randomUUID()
    val personaId = UUID.randomUUID()
    val latency = 1200L
  ...
  @Test
  fun sendPrompt_recordsErrorWhenInferenceFails() = runTest {
    val threadId = UUID.randomUUID()
    val personaId = UUID.randomUUID()
  ...
  @Test
  fun sendPrompt_prefersCloudWhenUserRequests() = runTest {
    val threadId = UUID.randomUUID()
    val personaId = UUID.randomUUID()
  ...
  @Test
  fun sendPrompt_appliesPersonaTuning() = runTest {
    val personaId = UUID.randomUUID()
    val threadId = UUID.randomUUID()
  ...
  @Test
  fun switchPersona_updatesExistingThread() = runTest {
    val threadId = UUID.randomUUID()
  ...
  @Test
  fun switchPersona_createsNewThreadWhenRequested() = runTest {
  ...
  @Test
  fun getPersonaSwitchHistory_delegatesToRepository() = runTest {
    val threadId = UUID.randomUUID()
    val logs =
      listOf(
        PersonaSwitchLog(

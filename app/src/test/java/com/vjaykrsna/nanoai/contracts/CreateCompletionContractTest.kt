...
/**
 * Contract test for POST /v1/completions endpoint.
 * Validates request/response schemas conform to contracts/llm-gateway.yaml OpenAPI spec.
 *
...
class CreateCompletionContractTest {
    ...
    fun `completion request should validate role enum values`() {
        ...
        invalidRoles.forEach { role ->
            assertThat(role).isNotIn(validRoles)
        }
    }
    ...
    fun `completion request should validate max_tokens minimum`() {
        ...
        invalidMaxTokens.forEach { maxTokens ->
            assertThat(maxTokens).isLessThan(1)
        }
    }
    ...
    fun `completion response should validate choice message role`() {
        ...
        invalidRoles.forEach { role ->
            assertThat(role).isNotEqualTo("assistant")
        }
    }

...
class ObserveUserProfileUseCaseTest {
  ...
  fun `merges dao and datastore flows and hydrates from cache`() = runTest {
    ...
    val useCase =
      instantiateUseCase(
        className = "com.vjaykrsna.nanoai.core.domain.uiux.ObserveUserProfileUseCase",
  ...
  private fun getReference(result: Any, vararg propertyCandidates: String): Any? {
    propertyCandidates.forEach { name ->
      runCatching {
          return UiUxDomainReflection.getProperty(result, name)
        }
        .getOrNull()
        ?.let {
          return it
  ...
  private fun getBoolean(result: Any, vararg propertyCandidates: String): Boolean {
    propertyCandidates.forEach { name ->

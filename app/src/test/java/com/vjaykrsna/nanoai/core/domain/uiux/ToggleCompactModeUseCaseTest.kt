...
class ToggleCompactModeUseCaseTest {
  ...
  fun `persists compact mode preference and updates layout snapshots`() = runTest {
    ...
    val useCase =
      instantiateUseCase(
        className = "com.vjaykrsna.nanoai.core.domain.uiux.ToggleCompactModeUseCase",
    ...
    val compactPrefs =
      spy.preferencesFlow.value ?: fail("Expected preferences emission for compact mode")
    ...
    val defaultPrefs =
      spy.preferencesFlow.value ?: fail("Expected preferences emission for default mode")

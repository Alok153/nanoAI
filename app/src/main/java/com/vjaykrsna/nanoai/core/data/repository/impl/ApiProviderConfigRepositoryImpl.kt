...
class ApiProviderConfigRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun getAllProviders(): List<APIProviderConfig> = apiProviderConfigDao.getAll().map { it.toDomain() }

        override suspend fun getProvider(providerId: String): APIProviderConfig? = apiProviderConfigDao.getById(providerId)?.toDomain()
        ...
        override suspend fun getEnabledProviders(): List<APIProviderConfig> = apiProviderConfigDao.getEnabled().map { it.toDomain() }

        override fun observeAllProviders(): Flow<List<APIProviderConfig>> =
            apiProviderConfigDao.observeAll().map { providers ->

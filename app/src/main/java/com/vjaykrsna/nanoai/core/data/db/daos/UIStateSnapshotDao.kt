 ...
 *
 * Provides reactive Flow-based operations for session restoration,
 * sidebar state management, and recent actions tracking.
 */
@Dao
interface UIStateSnapshotDao {
     ...
     */
    @Update
    suspend fun update(snapshot: UIStateSnapshotEntity): Int
    ...
    suspend fun updateSidebarCollapsed(
        userId: String,
        collapsed: Boolean,
    ): Int
    ...
    suspend fun updateExpandedPanels(
        userId: String,
        expandedPanels: List<String>,
    ): Int
    ...
    suspend fun updateRecentActions(
        userId: String,
     ...
     *
     * This transaction reads the current expanded panels, adds the new panel if not present,
     ...
     *
     * This transaction reads the current expanded panels, removes the panel if present,
     * and updates the list.
     *
     * @param userId The unique user identifier
     ...
     */
    @Transaction
    suspend fun removeExpandedPanel(
        userId: String,
        panelId: String,
    ) {
        val current = getByUserId(userId)
     ...
     *
     * This transaction reads the current recent actions, adds the new action at the front,
     * removes it from later positions if present (dedupe), truncates to max 5 items,
     * and updates the list.
     *
     * @param userId The unique user identifier

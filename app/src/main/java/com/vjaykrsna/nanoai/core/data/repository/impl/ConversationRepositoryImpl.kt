...
class ConversationRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun getThread(threadId: UUID): ChatThread? = chatThreadDao.getById(threadId.toString())?.toDomain()

        override suspend fun getAllThreads(): List<ChatThread> = chatThreadDao.getAllActive().map { it.toDomain() }

        override suspend fun getArchivedThreads(): List<ChatThread> = chatThreadDao.getArchived().map { it.toDomain() }

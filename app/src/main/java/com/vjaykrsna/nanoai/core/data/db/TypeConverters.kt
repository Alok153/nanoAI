...
class TypeConverters {
    ...
    @TypeConverter
    fun toThemePreference(name: String?): ThemePreference? = name?.let { ThemePreference.fromName(it) }

    @TypeConverter
    fun fromVisualDensity(visualDensity: VisualDensity?): String? = visualDensity?.name

    @TypeConverter
    fun toVisualDensity(name: String?): VisualDensity? =
        name?.let { candidate -> VisualDensity.values().firstOrNull { it.name.equals(candidate, ignoreCase = true) } }
            ?: VisualDensity.DEFAULT

    @TypeConverter
    fun fromScreenType(screenType: ScreenType?): String? = screenType?.name

    @TypeConverter
    fun toScreenType(name: String?): ScreenType? =
        name?.let { candidate -> ScreenType.values().firstOrNull { it.name.equals(candidate, ignoreCase = true) } }
            ?: ScreenType.UNKNOWN
    ...
    fun toStringList(value: String?): List<String> =
        value?.takeIf { it.isNotBlank() }?.let { payload -> json.decodeFromString(stringListSerializer, payload) }
            ?: emptyList()
    ...
    fun toStringBooleanMap(value: String?): Map<String, Boolean> =
        value
            ...
            ?.let { payload -> json.decodeFromString(stringBooleanMapSerializer, payload) }
            ?: emptyMap()

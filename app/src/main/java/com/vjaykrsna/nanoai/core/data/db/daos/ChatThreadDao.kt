...
interface ChatThreadDao {
    ...
    suspend fun insert(thread: ChatThreadEntity)
     ...
     */
    @Update
    suspend fun update(thread: ChatThreadEntity)
    ...
    /**
     * Get all threads ordered by update time (most recent first).
     * Excludes archived threads by default.
     */
     ...
     */
    @Query("SELECT COUNT(*) FROM chat_threads WHERE is_archived = 0")

...
data class LayoutSnapshot(
    ...
    init {
        ...
        require(name.length <= MAX_NAME_LENGTH) { "LayoutSnapshot name must be ≤ $MAX_NAME_LENGTH characters." }
    ...
    fun withPinnedTools(tools: List<String>): LayoutSnapshot = copy(pinnedTools = sanitizePinnedTools(tools, isCompact))
...
fun Flow<List<LayoutSnapshotRecord>>.mapToLayoutSnapshots(): Flow<List<LayoutSnapshot>> =
    map { records -> records.map(LayoutSnapshotRecord::toDomain) }

fun Flow<LayoutSnapshotRecord?>.mapToLayoutSnapshot(): Flow<LayoutSnapshot?> = map { record -> record?.toDomain() }

private fun sanitizePinnedTools(
    tools: List<String>,
    isCompact: Boolean,
): List<String> {

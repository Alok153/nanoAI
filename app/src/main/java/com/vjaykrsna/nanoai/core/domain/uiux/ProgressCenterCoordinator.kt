...
constructor(
  ...
  val progressJobs: Flow<List<ProgressJob>> =
    combine(observeDownloadJobs(), observeOtherJobs()) { downloads, otherJobs ->
        (downloads + otherJobs).distinctBy(ProgressJob::jobId).sortedBy { it.jobId.toString() }
      }
   ...
   * job observations.
   */
  private fun observeOtherJobs(): Flow<List<ProgressJob>> = progressRepository.progressJobs

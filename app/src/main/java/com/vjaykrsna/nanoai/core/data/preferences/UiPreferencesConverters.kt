...
/**
 * Helper responsible for encoding and decoding complex preference payloads using Kotlin Serialization.
 */
@Singleton
class UiPreferencesConverters(
    ...
    fun decodeStringList(payload: String?): List<String> =
        payload
            ...
            ?.let { json.decodeFromString(stringListSerializer, it) }
            ?: emptyList()

    fun encodeBooleanMap(values: Map<String, Boolean>): String = json.encodeToString(stringBooleanMapSerializer, values)

    fun decodeBooleanMap(payload: String?): Map<String, Boolean> =
        payload
            ?.takeIf { it.isNotBlank() }
            ?.let { json.decodeFromString(stringBooleanMapSerializer, it) }
            ?: emptyMap()
}

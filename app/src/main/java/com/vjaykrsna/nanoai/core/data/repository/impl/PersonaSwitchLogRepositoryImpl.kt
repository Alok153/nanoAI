...
constructor(private val personaSwitchLogDao: PersonaSwitchLogDao) : PersonaSwitchLogRepository {
    ...
    override suspend fun getSwitchHistory(threadId: UUID): List<PersonaSwitchLog> {
        return personaSwitchLogDao.getByThreadId(threadId.toString()).map { it.toDomain() }
    }

    override suspend fun getLatestSwitch(threadId: UUID): PersonaSwitchLog? {
        return personaSwitchLogDao.getLatestForThread(threadId.toString())?.toDomain()
    }

    override suspend fun getLogsByThreadId(threadId: UUID): Flow<List<PersonaSwitchLog>> {
        return personaSwitchLogDao.observeByThreadId(threadId.toString()).map { logs ->

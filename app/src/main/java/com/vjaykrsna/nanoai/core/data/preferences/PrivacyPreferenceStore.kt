...
class PrivacyPreferenceStore @Inject constructor(@ApplicationContext private val context: Context) {
     ...
     */
    val privacyPreference: Flow<PrivacyPreference> = context.dataStore.data.map { preferences ->
        PrivacyPreference(
            ...
            consentAcknowledgedAt = preferences[KEY_CONSENT_ACKNOWLEDGED_AT]?.let {
                Instant.fromEpochMilliseconds(it)
            },
            retentionPolicy = preferences[KEY_RETENTION_POLICY]?.let {

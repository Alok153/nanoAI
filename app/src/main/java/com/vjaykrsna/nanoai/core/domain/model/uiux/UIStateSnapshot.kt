...
data class UIStateSnapshot(
    ...
    fun withExpandedPanels(panels: List<String>): UIStateSnapshot = copy(expandedPanels = sanitizePanels(panels))

    fun withRecentActions(actions: List<String>): UIStateSnapshot = copy(recentActions = sanitizeRecentActions(actions))
...
fun Flow<UIStateSnapshotRecord?>.mapToUiStateSnapshot(): Flow<UIStateSnapshot?> = map { record -> record?.toDomain() }

fun Flow<UIStateSnapshotRecord?>.requireUiStateSnapshot(fallback: () -> UIStateSnapshot): Flow<UIStateSnapshot> =
    map { record -> record?.toDomain() ?: fallback() }

...
@Singleton
class SendPromptAndPersonaUseCase @Inject constructor(
    ...
    suspend fun sendPrompt(threadId: UUID, prompt: String, personaId: UUID): Result<Unit> {
        ...
        val generationOptions = persona?.let {
            GenerationOptions(
        ...
        val preferLocal = when {
            hasLocalModel && !isOnline -> true
            hasLocalModel -> true
            else -> false
        }

        val inferenceResult = inferenceOrchestrator.generateResponse(
            prompt = prompt,
            personaId = personaId,
    ...
    suspend fun switchPersona(
        ...
        val targetThreadId = when (action) {
    ...
    suspend fun getPersonaSwitchHistory(threadId: UUID): Flow<List<PersonaSwitchLog>> {
        return personaSwitchLogRepository.getLogsByThreadId(threadId)
    }
}

class OfflineNoModelException : IllegalStateException("Device offline with no local model available")

class InferenceFailedException(val code: String, message: String?) : IllegalStateException(

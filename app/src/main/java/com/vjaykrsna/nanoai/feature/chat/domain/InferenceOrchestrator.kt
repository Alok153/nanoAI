...
class InferenceOrchestrator @Inject constructor(
    ...
    private suspend fun runLocalInference(
        ...
        return localModelRuntime.generate(request)
            .fold(
                onSuccess = { result ->
    ...
    private suspend fun runCloudInference(
        ...
        val cloudModelId = options.cloudModel
            ?: modelCatalogRepository.getAllModels()
        ...
        val request = CompletionRequestDto(
            model = cloudModelId,
            messages = messages,
        ...
        return when (val result = cloudGatewayClient.createCompletion(provider, request)) {
            is CloudGatewayResult.Success -> {
                val choice = result.data.choices.firstOrNull()
                    ?: return InferenceResult.Error(

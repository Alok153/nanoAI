...
class ImportServiceImpl
    ...
    constructor(
        ...
        private suspend fun importPersonas(personas: List<BackupPersonaDto>): Pair<Int, Int> {
            ...
            for (dto in personas) {
                val personaId = dto.id?.let { runCatching { UUID.fromString(it) }.getOrNull() } ?: UUID.randomUUID()
        ...
        private fun buildNewProvider(
            providerId: String,
            dto: BackupApiProviderDto,
        ): APIProviderConfig =
            APIProviderConfig(
        ...
        private fun parseInstant(value: String): Instant = runCatching { Instant.parse(value) }.getOrElse { Clock.System.now() }

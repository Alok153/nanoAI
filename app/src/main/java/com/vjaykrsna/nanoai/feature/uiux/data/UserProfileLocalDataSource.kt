 ...
 *
 * Coordinates access to Room database (via DAOs) and DataStore (via UiPreferencesStore)
 * to provide a unified interface for local user profile data. Handles cache hydration
 * and preference overlay merging.
 */
@Singleton
class UserProfileLocalDataSource
    ...
    constructor(
         ...
         *
         * Combines database profile with DataStore preferences, with preferences taking
         * precedence for quick-access UI settings. Returns null if no profile exists.
         *
         ...
         */
        suspend fun updateLastOpenedScreen(
            userId: String,
         ...
         */
        suspend fun saveLayoutSnapshot(
            userId: String,
        ...
        fun observeUIStateSnapshot(userId: String): Flow<UIStateSnapshot?> =
            uiStateSnapshotDao.observeByUserId(userId).map { entity ->
                entity?.toDomain()
            }
         ...
         */
        suspend fun getUIStateSnapshot(userId: String): UIStateSnapshot? = uiStateSnapshotDao.getByUserId(userId)?.toDomain()

...
class DownloadManagerImpl
    ...
    constructor(
        ...
        override suspend fun getDownloadStatus(taskId: UUID): DownloadTask? = downloadTaskDao.getById(taskId.toString())?.toDomain()
        ...
        override suspend fun getActiveDownloads(): Flow<List<DownloadTask>> =
            downloadTaskDao.observeActiveDownloads().map { tasks ->
                tasks.map {
                    it.toDomain()
                }
            }
        ...
        override fun getQueuedDownloads(): Flow<List<DownloadTask>> =
            downloadTaskDao.observeQueuedDownloads().map { tasks ->
                tasks.map {
                    it.toDomain()
                }
            }

        override fun observeProgress(taskId: UUID): Flow<Float> = downloadTaskDao.observeById(taskId.toString()).map { it?.progress ?: 0f }

        override suspend fun getMaxConcurrentDownloads(): Int = DownloadManager.DEFAULT_MAX_CONCURRENT_DOWNLOADS
        ...
        override suspend fun getModelIdForTask(taskId: UUID): String? = downloadTaskDao.getModelIdForTask(taskId.toString())
        ...
        override suspend fun deletePartialFiles(modelId: String) {
            ...
            modelsDir.listFiles { file -> file.name.startsWith(modelId) }?.forEach { it.delete() }
        }

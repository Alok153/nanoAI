...
constructor(
    ...
    override suspend fun getDownloadStatus(taskId: UUID): DownloadTask? {
        return downloadTaskDao.getById(taskId.toString())?.toDomain()
    }

    override suspend fun getTaskById(taskId: UUID): Flow<DownloadTask?> {
        return downloadTaskDao.observeById(taskId.toString()).map { it?.toDomain() }
    }
    ...
    override suspend fun getMaxConcurrentDownloads(): Int {
        return DownloadManager.DEFAULT_MAX_CONCURRENT_DOWNLOADS
    }
    ...
    override suspend fun getModelIdForTask(taskId: UUID): String? {
        return downloadTaskDao.getModelIdForTask(taskId.toString())
    }

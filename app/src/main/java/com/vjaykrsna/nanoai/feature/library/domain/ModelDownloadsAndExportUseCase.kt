 ...
 */
@Singleton
class ModelDownloadsAndExportUseCase @Inject constructor(
    ...
    suspend fun downloadModel(modelId: String): Result<UUID> = runCatching {
        ...
        val taskId = if (activeCount >= maxConcurrent) {
    ...
    suspend fun verifyDownloadChecksum(modelId: String): Result<Boolean> = runCatching {
        val model: ModelPackage = modelCatalogRepository.getModelById(modelId).first()
            ?: throw IllegalStateException("Model $modelId not found")
        val expectedChecksum = model.checksum ?: run {
            modelCatalogRepository.updateInstallState(modelId, InstallState.ERROR)
            return@runCatching false
        }
        val actualChecksum = downloadManager.getDownloadedChecksum(modelId) ?: run {
            modelCatalogRepository.updateInstallState(modelId, InstallState.ERROR)
            return@runCatching false
        }
    ...
    suspend fun exportBackup(
        ...
        val bundlePath = exportService.createExportBundle(personas, providers, destinationPath, chatHistory)
...
class ModelInUseException(modelId: String) : IllegalStateException("Model $modelId is active in a conversation")

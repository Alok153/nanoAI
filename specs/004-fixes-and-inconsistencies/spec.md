...
- **FR-008 (Dead code & cleanup)**: Remove or document dead/unused code (e.g., unused `savedStateHandle` in `WelcomeViewModel.kt`) and remove hardcoded URLs or secrets. Each removal must include a short rationale in the commit message.
